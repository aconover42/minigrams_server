...
//    DWORD dwOptions_rec = DMA_FROM_DEVICE | DMA_ALLOW_CACHE | DMA_ALLOW_64BIT_ADDRESS;
    ...
    printf("     (27) PC XMIT TRIG readout \n");
...
/** try to cover  100ms */
   ...
   case 4:
     //user provides number of loops, stored to location of nloop
     printf(" number of loop \n");
     scanf("%d",&nloop);
     //user provides number of words contained in each packet, stored to location of nword
     printf(" enter number of word per packet\n");
     scanf("%d",&nword);

     //toggle printint, stored to iprint
     printf(" type 1 to print \n");
     scanf("%d",&iprint);
     ireadback =1;
     //toggle usage of random number, stored to irand
     printf(" type 1 to use random number \n");
     scanf("%d",&irand);
     //unclear what ireadback and inew do
     inew =1;
/*
     printf(" type 1 to use new PCie FPGA code \n");
     scanf("%d",&inew);
...
*/
     //create pointer pointing to location of buf_send
     px = &buf_send;
     imod =12;  /* set avoid the DCM  **/
     ichip=6;
     //loop with frequency set by nloop (given by user at beginning)
     for (j=0; j<nloop; j++) {
/** initialize **/
      if(j ==0) {
	//in initial loop, set first two positions to 0 in hexadecimal
       buf_send[0]=0x0;
       buf_send[1]=0x0;
       i=1;
       k=1;
       //unclear, possibly sending information to pcie card, unclear which card and what data
       i = pcie_send(hDev, i, k, px);
      }
      buf_send[0]=0x1;
      buf_send[1]=0x0;
      ...
      //for loop acts as a wait
      for (k=0; k<100000; k++) {    /* 100000 for 100 microsec */
       ik=k+1;
       i=ik*ik;
      }
      //checks that both ireadback and inew are set to one (they should be, don't think they could have changed yet)
      if(ireadback == 1) {
        if(inew == 1) {
	  //begins writing to addresses
         dwAddrSpace =2;
         u32Data = 0xf0000008;
         dwOffset = 0x28;
         WDC_WriteAddr32(hDev, dwAddrSpace, dwOffset, u32Data);
        ...
        WDC_WriteAddr32(hDev, dwAddrSpace, dwOffset, u32Data);
        dwAddrSpace =2;
	//stores # of characters per packet (4 characters per word, n words per packet)
        u32Data = 0x40000000+nword*4;
        dwOffset = 0x1c;
        WDC_WriteAddr32(hDev, dwAddrSpace, dwOffset, u32Data);
        dwAddrSpace =2;
...
/*      }   */
/** test command 1 ***/
      //for each word in the packet, execute the following code
      for (i=0; i<nword; i++) {
       ik=i%4;
       //set value of the corresponding location in send_array, based on position in group of 4 and whether irand is set
       //we understand how this for loop works and what it does, but not why we are doing this
       send_array[i]=0x5555aaaa ;
       if(ik == 0) send_array[i]= 0xaaaa5555+i;
       if(irand == 1) send_array[i]=rand();
       if(i ==0) send_array[i]=0xffffffff;
...
/*       send_array[i]= i+1;    */
      ...
      k= nword;
/** try to cover  100ms */
//i is set to pcie)send(hDev=which pcie card, i=1, k=number of words, px=buf_send (what we just finished filling))
      i = pcie_send(hDev, i, k, px);
      if(ireadback == 1) {
       dwAddrSpace =2;
       u64Data =0;
...
//     scanf("%d",&nloop);
     for ( iv=0; iv<nloop; iv++) {
      usleep(10000); // wait for 10ms
      inpf = fopen("/home/ub/feb_pmt_fpga_test","r");
      imod=11;
      ichip=mb_feb_conf_add;
      buf_send[0]=(imod<<11)+(ichip<<8)+0x0+(0x0<<16);  // turn conf to be on
      i=1;
...
//
      imod=11;
      ichip=3;
      buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_blocksize+(0xff0<<16);  // set max block size
      i=1;
      k=1;
      i = pcie_send(hDev, i, k, px);
...
case 27:
      // copy of case 4 from nevispcie_run.cpp, copied as-is, variables and info will be updated
      printf("*****************************************************\n");
      ...
      //MicroBoon neutrino events readout -- multiple boards -- case 33
      //mbtest_s5_sep24 case 3
      ...
      printf("\t(enter 1 if this is a calibration test)\t");
      scanf("%d",&itrig_c);
      //      itrig_c=1;
      ...
      scanf("%d",&nmod);
      nmod = 2; // 16/07/21: currently only 2 FEMs installed
      
      //      printf("\n\tXMIT slot number (count from right, starting at 1)?\t");
      //      scanf("%d",&imod_xmit);

      imod_xmit=6; // 16/07/21:need to confirm this
	...
	printf("%d\t",imod_xmit+1+i);
      ...
      if (itrig_c==1){
	//	printf("\n\tTrig board slot number?\t");
	//	scanf("%d",&imod_trig);
	imod_trig=12; // 16/07/21: need to confirm this
      }
      ...
      printf("\n\tUse ADC data (instead of fake FEM data)?\t");
      scanf("%d",&adcdata);
      //      adcdata=1;

      if (adcdata==1){
	printf("\n\tUse fake (a33) ADC data?\t");
	scanf("%d",&fakeadcdata);
	//	fakeadcdata=1;
      } else {
	fakeadcdata=0;
	...
	scanf("%d",&irand);
      ...
      scanf("%d",&ihuff);
      //      ihuff =0; //xxxxx need to predict event size 
	...
	if (iwritem==0){
	  ...
	  //	  dmasizewrite=1;
	  sprintf(name,"/data/dab/xmit_trig_bin_%i.dat",tnum);
	  fd = creat(name,0755);
	  printf("\n\tOutput file: %s\n",name);
	}
      }
      ...
      if (iwrited==1){
	sprintf(name,"/data/dab/xmit_trig_%i.out",tnum);
	outf = fopen(name,"w");
	printf("\n\tOutput file: %s\n",name);
      }
      ...
      scanf("%d",&idebug);
      //      idebug=0;
      printf("\n\tType 1 to check data during running:\t");
      scanf("%d",&irawprint);
      icompare=0;
      //      irawprint=0;
	...
	i = pcie_send(hDev, i, k, px);
      ...
      //inpf = fopen("/home/daquser/readout_config_files/readcontrol_11601_v3_play_header_6_21_2013.rbf","r");
      inpf = fopen("/home/daquser/readout_config_files/xmit_fpga_081913","r");
      imod=imod_xmit;
      ichip=mb_xmit_conf_add;
      buf_send[0]=(imod<<11)+(ichip<<8)+0x0+(0x0<<16);  // turn conf to be on
      i=1;
	...
	//inpf = fopen("/home/daquser/readout_config_files/module1x_120229_m_5_2_2012_nostp.rbf","r");
	inpf = fopen("/home/daquser/readout_config_files/feb_fpga_111913","r");
	...
	if (adcdata==1){
	  ...
	  for (is=0; is<8; is++) {
	    ...
	    buf_send[0]=(imod<<11)+(ichip<<8)+(mb_pmt_adc_data_load)+(0x0300<<16); //1st next word will be overwrite by the next next word
	    //	      buf_send[1]=(((0x0)<<13)+(0xd))+((0x9)<<24)+((0x0)<<16);
	    if (fakeadcdata==1) buf_send[1]=(((0x0)<<13)+(0xd))+((0xc)<<24)+((0x0)<<16);
	    else buf_send[1]=(((0x0)<<13)+(0xd))+((0x0)<<24)+((0x0)<<16); //0x0<<24 is for baseline //  set /w =0, w1,w2 =0, a12-a0 = 0xd, data =0xc (mixed frequency);
	    i=1;
	    k=2;
	    i = pcie_send(hDev, i, k, px);
	    usleep(5000);   // sleep for 2ms
...
/*       i = pcie_send(hDev, i, k, px); */
	...
	nword = (((64*timesize*3)/2+5)*(imod_st-imod_xmit))+2; 
      ...
      set=0;
       ...
       /** begin timer **/
       struct timeval starttest331, endtest331;
       gettimeofday(&starttest331,NULL);
       long mytime331, seconds331, useconds331;
       seconds331 = starttest331.tv_sec;
       useconds331 = starttest331.tv_usec;
      
       printf("\n\nStart time of test: %ld sec %ld usec\n",seconds331,useconds331);
	...
	for (iv=0; iv<(ndma_loop+1); iv++) {
	  ...
	  if((iv%2) == 0) buffp_rec32 = (UINT32*)pbuf_rec1;
	  else buffp_rec32 = (UINT32*)pbuf_rec2;
	  ...
	  WDC_WriteAddr32(hDev2, dwAddrSpace, dwOffset, u32Data);
	  if(idebug ==1) printf(" DMA set up done, byte count = %d\n", nwrite_byte);
	  ...
	  if(idone == 0) {
	    ...
	    if (iwritem==1){
	      sprintf(name,"/data/dab/xmit_trig_bin_dma_%i_%i_%i.dat",tnum,ijk,iv);
	      fd = creat(name,0755);
	...
	if (irawprint==1){
	  ...
	  //check if first event looks okay 
	  if ( ihuff==1 && ((icompare==1 && ijk==0) || (icompare==0))){//first event, if COMPRESSION!!
		...
		if ((read_array_compare[ic]&0xffff)==0xffff && (notadcdata==1)){
		  ...
		  if ( ncount<=nwords ){ //while reading lines for this event
		    ...
		    if ( ncount==nwords+1 ) notadcdata=1;
	  ...
	  if (icompare==1 && ijk!=0){
	    ...
	    if ((ijk%10)==0) printf("Checked %d events.\n",ijk);
       ...
       gettimeofday(&endtest331,NULL);
       seconds331 = endtest331.tv_sec;
       useconds331 = endtest331.tv_usec;

       printf("\nEnd time of test:   %ld sec %ld usec\n",seconds331,useconds331);

       seconds331 = endtest331.tv_sec - starttest331.tv_sec;
       useconds331 = endtest331.tv_usec - starttest331.tv_usec;

       mytime331 = seconds331*1000000. + useconds331*1.; //elapsed time in usec
...
*/
  ...
  int newcmd,itr,tr_bar,       rr_bar,
    rt_cs_reg,
    rr_cs_reg,
    icheck,istop,iloop,t_cs_reg,r_cs_reg;
  ...
  printf(" type 1 to use random number \n");
  scanf("%d",&irand);
  printf(" which transceiver to test (1,2,12) \n");
  printf(" 12 is loop back across tranceivers \n");
  scanf("%d", &itr);
...
/* clear DMA register after the abort */
	...
	//       printf("started loop = %d \n",j);
      ...
      WDC_WriteAddr32(hDev, dwAddrSpace, dwOffset, u32Data);
      //GSK
      //       if(iprint == 1 && j < 1) printf(" set up transmitter \n");
	...
	if(icheck == 1) {
      ...
      u32Data = 0x22211; 
      if (j < 1 && i != 12)
	printf("Writing data across the same receiver/transmitter...");
      if (j < 1 && i == 12) 
	printf("Writing data across tranceiver 1 and 2");
	...
	printf("Writing word 0x%08x",u32Data);	
	...
	if((iprint ==1) && (nwrite%8 !=0) && j < 1) printf("\n");
      }
...
/** start the receiver **/
      ...
      WDC_WriteAddr32(hDev, dwAddrSpace, dwOffset, u32Data);
       //GSK
       //       if(iprint == 1 && j < 1) printf(" set up receiver \n");
       //       if(icheck == 1) {
       //         dwAddrSpace =cs_bar;
       //         u64Data =0;
       //         dwOffset = t_cs_reg;
...
/***    check to see if DMA is done or not **/
	...
	WDC_ReadAddr32(hDev, dwAddrSpace, dwOffset, &u32Data);
	  //GSK
	  //	  if(iprint ==1 && j < 1) printf(" send DMA status word %d %X \n", i, u32Data);
	if((u32Data & dma_in_progress) == 0) {
	    //GSK
	    //	    if(iprint == 1 && j < 1) printf(" send DMA complete %d \n", i);
	  idone =1;
	}
	if((u32Data & dma_in_progress) == 0) break;
      }
...
/***    check to see if DMA is done or not **/
	...
	WDC_ReadAddr32(hDev, dwAddrSpace, dwOffset, &u32Data);
	//GSK
	//	  if(iprint ==1 && j < 1) printf(" receive DMA status word %d %X \n", i, u32Data);
	if((u32Data & dma_in_progress) == 0) {
	  idone =1;
	  //GSK
	  //	    if(iprint == 1 && j < 1) printf(" receive DMA complete %d \n", i);
	}
	if((u32Data & dma_in_progress) == 0) break;
      }
      /* synch DMA i/O cache **/
	 ...
	 //	   dwOffset = cs_dma_cntrl;
	 //           WDC_ReadAddr32(hDev, dwAddrSpace, dwOffset, &u32Data);
	 //	   if(iprint ==1 && j < 1) printf(" receive DMA status word after abort %d %X \n", i, u32Data);
	 //           dwAddrSpace =cs_bar;
	 //           dwOffset = t_cs_reg;
	 //           WDC_ReadAddr64(hDev, dwAddrSpace, dwOffset, &u64Data);
	 //	   if(iprint==1 && j < 1) {
